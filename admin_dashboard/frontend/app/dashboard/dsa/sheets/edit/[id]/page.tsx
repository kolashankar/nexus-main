...
export default function EditDSASheet() {
  ...
  const fetchSheet = async () => {
    try {
      setFetching(true)
      const response = await dsaApi.sheets.getById(sheetId)
      const sheet = response.data.sheet
      setFormData({
        name: sheet.name || '',
        description: sheet.description || '',
        author: sheet.author || '',
        level: sheet.level || 'beginner',
        tags: sheet.tags?.join(', ') || '',
        questions: sheet.questions || [],
        is_published: sheet.is_published ?? false,
      })
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await dsaApi.sheets.update(sheetId, payload)
      toast.success('Sheet updated successfully!')
      router.push('/dashboard/dsa/sheets/list')
    } catch (error: any) {
      console.error('Error updating sheet:', error)
      toast.error(error.response?.data?.detail || 'Failed to update sheet')
    } finally {
      setLoading(false)
    }
  }
  ...
  if (fetching) {
    return (
      <div className="p-6">
        <div className="text-center py-12">Loading sheet data...</div>
      </div>
    )
  }
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-2">Edit DSA Sheet</h1>
        <p className="text-gray-600">Update sheet information</p>
      </div>
