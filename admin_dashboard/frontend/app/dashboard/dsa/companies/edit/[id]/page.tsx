...
export default function EditDSACompanyPage() {
  const params = useParams()
  const [company, setCompany] = useState(null)
  const [loading, setLoading] = useState(true)
  useEffect(() => { const load = async () => { try { const res = await fetch(`/api/admin/dsa/companies/${params.id}`); setCompany(await res.json()) } catch (e) { alert('Failed') } finally { setLoading(false) } }; if (params.id) load() }, [params.id])
  if (loading) return <ModernDashboardLayout><div className="flex items-center justify-center h-96"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-slate-600"></div></div></ModernDashboardLayout>
  return <ModernDashboardLayout><div className="space-y-6"><div><Link href="/dashboard/dsa/companies/list" className="inline-flex items-center text-sm text-slate-600 mb-2"><ArrowLeft className="w-4 h-4 mr-1" />Back</Link><div className="flex items-center space-x-3"><div className="p-3 bg-gradient-to-br from-slate-500 to-gray-600 rounded-xl shadow-lg"><Building2 className="w-6 h-6 text-white" /></div><div><h1 className="text-3xl font-bold text-slate-900">Edit Company</h1></div></div></div>{company && <ModernDSACompanyForm initialData={company} isEditing />}</div></ModernDashboardLayout>
}
