...
export default function EditDSACompany() {
  ...
  const fetchCompany = async () => {
    try {
      setFetching(true)
      const response = await dsaApi.companies.getById(companyId)
      setFormData({
        ...
        problem_count: response.data.company.problem_count || 0,
        job_count: response.data.company.job_count || 0,
        is_active: response.data.company.is_active ?? true,
      })
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await dsaApi.companies.update(companyId, formData)
  ...
  if (fetching) {
    return (
      <div className="p-6">
        <div className="text-center py-12">Loading company data...</div>
      </div>
    )
  }
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-2">Edit DSA Company</h1>
